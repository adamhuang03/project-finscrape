...
class BetakitFundingScraper(BetakitScraper):
    """Build ontop of BetakitScraper to scrape this: https://betakit.com/tag/funding/
    ...
    def __init__(self, url, header) -> None:
        ...
        self.soup = self._soup_content(url=self.url)
        self._find_articles()
    ...
    async def _ai_filter_articles(self):
        ...
        while bad:
            x = []
            tasks = []
            for article_dict in self.articles.values():
                tasks.append(self._ai_comboText_search(article_dict['title_description']))
                x.append(article_dict['title_description'])
            results = await asyncio.gather(*tasks)
            results_content = [self.regex_aiValidCheck(text=res.content, value='1') for res in results]
            try:
                list(map(int, results_content))
                bad = False
            except:
                print(bad)
                print(results)
        ...
        print(x)
        self.filtered_articles = {
            idx: article_dict for idx, (article_dict, checker) in enumerate(zip(self.articles.values(), results_content)) if checker == 1
        }
    ...
    def _betakit_strip(self, elements: list) -> list:
        filtered_elements = [
            str(element).strip() for element in elements
            if element.parent.name not in ['script', 'title']
        ] #this is betakit specific
        return filtered_elements

    def _check_content(self, soup: bs, target_string: str):
        """This is an exact search
        """
        regex_elements = self._regex_search(soup=soup, target_string=target_string)

        return self._betakit_strip(elements=regex_elements)
    ...
    def _iterate_articles(self):
        for idx, dict_content in self.filtered_articles.items():
            ...
            elements = self._check_content(soup=soup_article, target_string='invest')
...
if __name__ == '__main__':
    ...
    main = BetakitFundingScraper(url=url, header=headers)
    # print(main)
